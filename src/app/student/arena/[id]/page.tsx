...
import { ArrowLeft, Code2, Play, AlertTriangle, Smile, Lock } from 'lucide-react';
...
function useSequentialTypewriter(text: string, speed: number = 8, startTrigger: boolean = true) {
  ...
  useEffect(() => {
    if (!text || !startTrigger || isFinished) return;
...
export default function DynamicArenaPage() {
  ...
  useEffect(() => {
    async function fetchTaskData() {
      try {
        ...
        if (result.success) {
          const data: TaskData = result.data.task || result.data;
          setTask(data);
          ...
          // 🌟 Əgər üst və ya alt kod varsa, redaktoru ona uyğun ilkin vəziyyətə gətiririk
          if (data.headerCode || data.footerCode) {
            setEditorValue(`// Kodunuzu bura yazın 📝`);
          } else {
  ...
  const goBackToMap = () => {
    playSFX('btn2', 0.5);
    switch (task?.level) {
      case 1: navigateTo('/student/learning'); break;
      case 2: navigateTo('/student/adventure'); break;
      default: navigateTo('/student/learning');
    }
  };
  ...
  const validateCode = async () => {
    ...
    // 🌟 Tam Kodu Hazırlayırıq: Üst Kod + Şagirdin Kodu + Alt Kod
    const fullCodeToRun = [
      task.headerCode || '',
      editorValue,
      task.footerCode || ''
    ...
    for (let i = 0; i < task.testCases.length; i++) {
      ...
      try {
        const response = await fetch("/api/compiler", {
          ...
          body: JSON.stringify({
            code: fullCodeToRun, // 🚀 Kompilyatora bütöv kod gedir
  ...
  const headerLineCount = task?.headerCode
    ? task.headerCode.trimEnd().split('\n').length
    : 0;

  // 2. Orta hissə (Şagirdin yazdığı kod) neçə sətirdən ibarətdir?
          ...
          <div className="w-full flex items-start gap-5 self-end flex-row-reverse max-w-[94%] animate-student-layout">
            ...
            <div className="flex-1 bg-white border-3 p-5 rounded-[28px] rounded-tr-none shadow-md space-y-4 text-right animate-bubble-in dynamic-student-box">
              <div className="text-left space-y-2">
                <span className="text-[10px] font-black uppercase tracking-wider block dynamic-student-label">💻 SƏNİN SEHRLİ C++ KODUN:</span>

                <div className="rounded-2xl overflow-hidden border-3 border-slate-200 shadow-sm bg-white task-editor">

                  {/* 🔒 1. HEADER CODE (ÜST KOD - QORUNAN / DƏYİŞDİRİLƏ BİLMƏYƏN HİSSƏ) */}
                  {task?.headerCode && (
                    <div className="border-b-2 border-slate-200 bg-slate-100/90 relative">
                      <CodeMirror
                        value={task.headerCode}
                        editable={false}
                        readOnly={true}
                        extensions={[cpp()]}
                  ...
                  <CodeMirror
                    value={editorValue}
                    height="100%"
                    minHeight={task?.headerCode || task?.footerCode ? "200px" : "350px"}
                    extensions={[
                      cpp(),
                      lineNumbers({
                        formatNumber: (lineNo) => String(lineNo + headerLineCount)
                      })
                    ]}
                  ...
                  {task?.footerCode && (
                    <div className="border-t-2 border-slate-200 bg-slate-100/90 relative">
                      <CodeMirror
                        value={task.footerCode}
                        editable={false}
                        readOnly={true}
                        extensions={[
                          cpp(),
                          lineNumbers({
                            formatNumber: (lineNo) => String(lineNo + headerLineCount + editorLineCount)
                          })
                        ]}
                        theme="light"
                        className="opacity-75 pointer-events-none"
